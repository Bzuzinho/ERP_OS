...
export default function RecurringOperationsCreate({ types, statuses, frequencies }: { types: string[]; statuses: string[]; frequencies: string[] }) {
    const form = useForm({
        title: '',
        description: '',
        operation_type: types[0] ?? 'task',
    ...
    return (
        <AdminLayout title="Nova Recorrência" subtitle="Criar operação recorrente">
            <form onSubmit={(e) => { e.preventDefault(); form.post(route('admin.recurring-operations.store')); }} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
                ...
                <button type="submit" className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Guardar Recorrência</button>
            </form>
        </AdminLayout>
    );
}
