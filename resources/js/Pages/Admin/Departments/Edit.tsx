...
type Department = {
    ...
    manager_id: number | null; is_active: boolean;
};

type Props = { department: Department; users: UserOption[] };

export default function DepartmentsEdit({ department, users }: Props) {
    const form = useForm({
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.put(route('admin.hr.departments.update', department.id));
    };

    return (
        <AdminLayout title="Editar Departamento" subtitle={department.name}>
            <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input value={form.data.slug} onChange={(e) => form.setData('slug', e.target.value)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm" />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <select value={form.data.manager_id} onChange={(e) => form.setData('manager_id', e.target.value)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm">
                ...
                <div className="flex items-center gap-3 pt-2">
                    ...
                    <Link href={route('admin.hr.departments.show', department.id)} className="text-sm text-slate-700 hover:text-slate-950">Cancelar</Link>
                </div>
            </form>
        </AdminLayout>
    );
