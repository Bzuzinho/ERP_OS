...
type AbsenceType = { id: number; name: string; slug: string; description: string | null; max_days_per_year: number | null; requires_proof: boolean; is_paid: boolean; color: string | null; is_active: boolean };
type Props = { type: AbsenceType };

export default function AbsenceTypesEdit({ type }: Props) {
    const form = useForm({
        name: type.name, slug: type.slug, description: type.description ?? '',
        max_days_per_year: type.max_days_per_year?.toString() ?? '', requires_proof: type.requires_proof,
        is_paid: type.is_paid, color: type.color ?? '#6366f1', is_active: type.is_active,
    });

    const submit = (e: FormEvent) => { e.preventDefault(); form.put(route('admin.hr.absence-types.update', type.id)); };

    return (
        <AdminLayout title="Editar Tipo de Ausência" subtitle={type.name}>
            <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input type="number" value={form.data.max_days_per_year} onChange={(e) => form.setData('max_days_per_year', e.target.value)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm" min={1} />
                    </div>
