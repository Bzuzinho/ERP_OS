    ...
    return (
        <PortalLayout title="Atas de Reunião">
            <div className="space-y-3">
                {minutes.data.map((m) => (
                    <div key={m.id} className="flex items-center justify-between rounded-2xl border border-stone-200 bg-white p-4">
                        ...
                        <Link href={route('portal.meeting-minutes.show', m.id)} className="rounded-xl border border-stone-300 px-3 py-1.5 text-sm text-stone-700 hover:bg-stone-50">
                            Ver
                        </Link>
                    </div>
                ))}
